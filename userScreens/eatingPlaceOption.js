...
export default function EatingPlaceOption({ navigation }) {
  const [eatingPlaces, setEatingPlaces] = useState([
    { place: "A", stores: { store1: { storeName: "Japanese Cuisine", waitTime: 12, items: {}}}, averageWaitTime: 20, key: "1" },
    { place: "B", stores: { "1": 13, "2": 16 }, averageWaitTime: 30, key: "2" },
    { place: "C", stores: { "1": 14, "2": 17 }, averageWaitTime: 40, key: "3" },
  ]);

  return (
    <View style={styles.container} >
      <Text>Eating places options</Text>
      <FlatList
        data={eatingPlaces}
        renderItem={({ item }) => (
          <FlatButton
